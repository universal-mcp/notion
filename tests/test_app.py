...
@pytest.fixture
def app_instance():
    """Provides a NotionApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return NotionApp(integration=mock_integration)

def test_universal_mcp_notion_app_initialization(app_instance):
    """
    Test that the NotionApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "notion", "NotionApp instance has unexpected name."


def test_universal_mcp_notion_tool_docstrings_format(app_instance):
    """
    Test that each tool method in NotionApp has a well-formatted docstring,
...
def test_universal_mcp_notion_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in NotionApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
